...
//    dimSizes.push_back(0);
  ...
  void applyPerm(std::vector<int>& perm) {
    if (valueArray.size()) {
      static victor<V> new_val_array;
      assert(perm.size() == valueArray.size());
      new_val_array.clear();
      new_val_array.resize0(perm.size());
...
//  ret->dimSizes.push_back(0);
  ret->dimSizes.push_back(rowStore->size), ret->dimSizes.push_back(colStore->size);
  ...
  std::cerr << "Read Time = " << (TI-tic) << std::endl;
  return ret;
}
...
bool SparlayStorage<V>::fuse(const int lv) {
  ...
  std::cerr << "Fuse Time = " << (TI-tic) << std::endl;
  return 1;
}
...
bool SparlayStorage<V>::separate(const int lv) {
  ...
  std::cerr << "Seperate Time = " << (TI-tic) << std::endl;
  return 1;
}
...
bool SparlayStorage<V>::vectorize(const int lv) {
  ...
  if (father_ptr.size()) {
    ...
    size_t capacity = father_crd.size() * (size_t)cur_lv_size;
    new_value.clear();
    ...
    for (size_t i = 0; i < father_crd.size(); ++i) {
      ...
      for (int j = prev_ptr; j < father_ptr[i+1]; ++j) {
        int idx = i*cur_lv_size + vLevel[lv]->crd[j];
        new_value[idx] = valueArray[j];
      }
    ...
    int* father_crd_obj = father_crd.data();
    int* crd_obj = vLevel[lv]->crd.data(); //maybe this can skip boundary check
    size_t sz = father_crd.size();
    for (size_t i = 0; i < sz; ++i) {
      int idx = r_father_crd[father_crd_obj[i]-mn_crd]*cur_lv_size + crd_obj[i];
      new_value[idx] = valueArray[i];
    }
    father_crd = std::move(new_crd);
    
    vLevel[lv-1]->same_path = std::move(new_same_path);
    this->singleVectorSize = cur_lv_size;
    valueArray = std::move(new_value);
    vLevel.pop_back();
    // modify pointer at level 0
    vLevel[0]->ptr[1] = father_crd.size();
  }
  return 1;
}
      ...
      for (size_t i = 0; i < father_crd.size(); ++i) {
        ...
        for (int j = prev_ptr; j < father_ptr[i+1]; ++j) {
          for (size_t k = 0; k < Size; ++k) {
            int idx = j*Size+k;
            if (valueArray[idx]!=0.0) {
              cnt++;
              new_crd.push_back(k);
              new_same_path.push_back(0);
              new_value.push_back(valueArray[idx]);
            }
          }
        }
...
#define IMPL_SPTSWAP(VNAME, V) \
  void* _mlir_ciface_sptSwap##VNAME(void* ptr, int LU, int LD) { \
      ...
      std::cerr << "Swap Time: " << (TI-tic) << std::endl; \
      return (void*)sparT; \
  }
...
#define IMPL_SPTSUB(VNAME, V) \
  void* _mlir_ciface_sptSub##VNAME(void* ptr, int Ltarget, int Lsrc) { \
      ...
      std::cerr << "Sub Time: " << (TI-tic) << std::endl; \
      return (void*)sparT; \
  }
...
#define IMPL_SPTMOVE(VNAME, V) \
  void* _mlir_ciface_sptMove##VNAME(void* ptr, int srcLv, int dstLv) { \
    ...
    std::cerr << "Move Time: " << (TI-tic) << std::endl; \
    return (void*)sparT; \
  }
...
#define IMPL_SPTVECTORIZE(VNAME, V) \
  void* _mlir_ciface_sptVectorize##VNAME(void* ptr, int lv) { \
    ...
    std::cerr << "Vectorize Time: " << (TI-tic) << std::endl; \
    return (void*)sparT; \
  }
...
#undef IMPL_NEWSPARLAYTENSOR
    ...
    void _mlir_ciface_kernel_dia_spmm(StridedMemRefType<float, 2> *outC,
        ...
        SparlayStorage<float>* spA = (SparlayStorage<float>*)inA;
    ...
    SparlayStorage<float>* sparT = (SparlayStorage<float>*)(ptr);
  ...
  void _mlir_ciface_kernel_bdia_spmv_iter(StridedMemRefType<DataType, 1> *outC,
        ...
        DataType sum;
        SparlayStorage<DataType>* spA_CSR = (SparlayStorage<DataType>*)inA_CSR;
        SparlayStorage<DataType>* spA_BDIA = (SparlayStorage<DataType>*)inA_BDIA;
  ...
  void* _mlir_ciface_decompose_BDIA(void* ptr, int32_t blockSize, float thres) {
    SparlayStorage<DataType>* sparT = (SparlayStorage<DataType>*)ptr;
    // int32_t *row_crd = sparT->vLevel[1]->crd.data();
    // int32_t *col_crd = sparT->vLevel[2]->crd.data();
    ...
    std::vector<float> values(sparT->valueArray.to_vector()); //FIXME: extra copy here
    ...
    auto T_BDIA = new SparlayStorage<DataType>();
  ...
  void* _mlir_ciface_decompose_BDIA_opt(void* ptr, int32_t blockSize, float thres) {
    ...
    sparT->valueArray.resize0(nnz-diag_nnz_count);
  ...
  void* _mlir_ciface_decompose_BDIA_opt2(void* ptr, int32_t blockSize, float thres) {
    ...
    std::vector<int> row_crd(sparT->vLevel[1]->crd);
    std::vector<int> col_crd(sparT->vLevel[2]->crd);
    std::vector<DataType> values(sparT->valueArray.to_vector());
    sparT->vLevel[0]->ptr.pop_back();
    sparT->vLevel[1]->crd.clear();
    sparT->vLevel[1]->same_path.clear();
    sparT->vLevel[2]->crd.clear();
