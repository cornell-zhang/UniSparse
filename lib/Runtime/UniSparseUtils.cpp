...
UniSparseStorage* readFromFile(std::istream& fin);

void UniSparseStorage::Print(std::ostream& fout, bool verbose) {
  fout << "==============================================" << std::endl;
  for (size_t i = 0; i < vLevel.size(); ++i) {
    fout << "crd[" << i << "]: ";
...
bool UniSparseStorage::moveLv(const int srcLv, const int dstLv) {
...
bool UniSparseStorage::tile_split(int lv, int factor) {
...
bool UniSparseStorage::tile_merge(int lv, int factor) {
  assert(!(vLevel[lv]->type & LVFUSE));
  assert(vLevel[lv]->type & LVTRIM);
 ...
 */
void UniSparseStorage::dfsLowerPtr(int cur_lv, int id, int pos, int target_lv, std::vector<int>& ret) {
  if (cur_lv == target_lv) {
...
std::vector<int> UniSparseStorage::lowerPtr(int st_lv, int ed_lv) {
  std::vector<int> ret;
  u64 ed_level_size = 1;
...
bool UniSparseStorage::enumerate(const int dst_lv, const int slice_lv) {
...
bool UniSparseStorage::sums(const int lv) { 
...
bool UniSparseStorage::reorder(const int dst_lv, const int slice_lv) {
...
bool UniSparseStorage::schedule(const int dst_lv, const int slice_lv, const int partitions) {
  int total_size = 1;
//  std::cerr << slice_lv << std::endl;
  for(int i = 1; i <= slice_lv; i++) {
    assert(!(vLevel[i]->type & LVFUSE));
...
bool UniSparseStorage::pad(const int start_lv, const int end_lv) {
...
bool UniSparseStorage::grow(const int lv) {
...
bool UniSparseStorage::trim(const int lv) {
...
bool UniSparseStorage::fuse(const int lv) {
...
bool UniSparseStorage::separate(const int lv) {
...
bool UniSparseStorage::swap(const int LU, const int LD) {
  assert(LU < LD);
  for (int i = LU; i <= LD; ++i) {
    assert(!(vLevel[i]->type & LVFUSE));
    assert(vLevel[i]->type&LVTRIM);
...
bool UniSparseStorage::add(const int Ltarget, const int Lsrc) {
  assert(Lsrc != Ltarget);
...
bool UniSparseStorage::sub(const int Ltarget, const int Lsrc) {
  assert(Lsrc != Ltarget);
...
bool UniSparseStorage::vectorize(const int lv) {
//  assert(lv == 2);
...
bool UniSparseStorage::devectorize(const int level) {
...
bool UniSparseStorage::neg(int lv) {
  assert(!(vLevel[lv]->type & LVFUSE));
#ifdef PARALLEL
  int* lv_crd = vLevel[lv]->crd.data();
...
bool UniSparseStorage::fuse_enumerate_pad(const int dst_lv, const int start_lv, const int end_lv) {
  #ifdef DEBUG
  auto tic = TI;
  #endif
...
//One shot write 
bool UniSparseStorage::cust_pad_opt(const int start_lv, const int end_lv) {
  for(size_t i = 1; i <= end_lv; i++) {
    assert(vLevel[i]->type & LVTRIM);
  }
...
//
bool UniSparseStorage::fuse_dia_2d_only(const int neg, const int keep_lv) {
...
bool UniSparseStorage::fuse_transpose(const int mode) {
  assert(vLevel.size() == 3);
  assert(vLevel[2]->type == LVTRIM);
...
bool UniSparseStorage::pack(const int start_lv, const int end_lv) {
  for(int i = start_lv; i < end_lv; i++) {
    assert(vLevel[i]->type = LVTRIM);
  }
...
bool UniSparseStorage::partition(const int lv) {
  return 1;
}
...
void genWindowBuffer(UniSparseStorage* T, UniSparseWindow* W) {
  assert(W->M[1][0] == 0);
  if (W->M[0][1]) {
    assert(W->M[0][0] == 0);
    assert(W->M[1][1] == 0);
...
float getWindowSize(UniSparseStorage* T, UniSparseWindow* W) {
  float n = 1.0, m = 1.0;
  if (W->M[0][0] && W->M[1][1]) {
...
float getMaxDensity(UniSparseStorage* T, UniSparseWindow* W) {
  genWindowBuffer(T, W);
...
void pushCrd(UniSparseStorage* T, int i, int j, float val) {
...
void emplaceCrd(UniSparseStorage* T, UniSparseWindow* W, int L, int R, std::vector<UniSparseStorage*>& cand, const std::vector<float>& thres, const float win_size, const float mx_density) {
  if (L > R) {
    assert(L == 0 && R == -1);
    return;
  }
...
void sortCrd(UniSparseStorage* T, UniSparseWindow* W, const std::vector<float>& thres, const float mx_density, std::vector<UniSparseStorage*>& cand) {
  genWindowBuffer(T, W);
  assert(T->vLevel.size() == (size_t)3);
  assert(T->vLevel[1]->crd.size() == T->vLevel[2]->crd.size());
  int prev_id = 0;
...
extern "C" {
  ...
  void _mlir_ciface_sptCheck(void* A, void* B) {
    ...
    UniSparseStorage* aa = (UniSparseStorage*)(A);
    UniSparseStorage* bb = (UniSparseStorage*)(B);
    if ((*aa) == (*bb)) {
  ...
  void* _mlir_ciface_sptCopy(void* A) {
    UniSparseStorage* aa = (UniSparseStorage*)(A);
    UniSparseStorage* ret = new UniSparseStorage();
    (*ret) = aa->copy();
    return (void*)ret;
  }
  ...
  void* _mlir_ciface_sptSplit(StridedMemRefType<float, 1>* thres, void* ptr, void* win) {
    ...
    std::vector<UniSparseStorage*> cand;
    for (int64_t i = 0; i < size + 1; ++i) {
      cand.push_back(new UniSparseStorage);
      UniSparseStorage* newT = new UniSparseStorage;
      (*newT) = sparT->copy();
...
//        printf("outdata[%d] is %f\n", i, out->data[i]);
  ...
  void delUniSparseTensor(void *tensor) {
    delete static_cast<UniSparseStorage *>(tensor);
  }

  uint64_t _mlir_ciface_sparseDimSize(void *tensor, uint64_t d) {
    return static_cast<UniSparseStorage *>(tensor)->getDimSize(d);
  }

  void _mlir_ciface_endInsert(void *tensor) {
//    std::cout << "Start endInsert " << std::endl;
    return static_cast<UniSparseStorage *>(tensor)->endInsert();
  }
...
//    std::cout << "Start lexInsert " << std::endl;        
    ...
    static_cast<UniSparseStorage *>(tensor)->lexInsert(cursor, *value);
  }
...
//    std::cerr << "Enter Expand Insert " << std::endl;                            
    static_cast<UniSparseStorage *>(tensor)->expInsert(               
        cursor, values, filled, added, count);                          
  }

  void* _mlir_ciface_newUniSparseTensor(StridedMemRefType<UniSparseDimLevelType, 1> *aref,
...
//    std::cout << "Create UniSparseStorage " << std::endl;
    auto *tensor = new UniSparseStorage(vshape, perm, sparsity);
    return tensor;
  }
