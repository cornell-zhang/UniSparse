...
long test_ops(MKL_INT rows, MKL_INT cols, MKL_INT *row_ptr, MKL_INT *col_idx, MKL_INT *row_ptr1) {
    long nnz = 0;
    for(int i = 0; i < rows; i++) {
//        std::cout << "Finish initialize" << std::endl;
        for(int j = row_ptr[i]; j < row_ptr[i+1]; j++) {
            int idx = col_idx[j];
            nnz = nnz + 2 * (row_ptr1[idx+1] - row_ptr1[idx]); 
        }
    }
    return nnz;
}
...
*/
 ...
 //   long ops = test_ops(input0.num_rows, input0.num_cols, input0.csrRowPtr, input0.csrColInd, input1.csrRowPtr);
