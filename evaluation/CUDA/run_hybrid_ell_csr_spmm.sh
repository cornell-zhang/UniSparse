make cusparse_hybrid_ell_csr_spmm
...
DATSETS1=(
        # nemeth21_row_major.mtx_CSR.npz
        # email-Eu-core_row_major.mtx_CSR.npz
...
for ((i=0;i<${#DATSETS1[@]};i++)); do
    echo ${BUILD_DIR}/cusparse_hybrid_ell_csr_spmm ${DATSETS1[i]} ${DATSETS2[i]}
    ${BUILD_DIR}/cusparse_hybrid_ell_csr_spmm $DATASET_PATH/${DATSETS1[i]} $DATASET_PATH/${DATSETS2[i]}
done
