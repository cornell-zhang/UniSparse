...
int benchmark_spmv_hybrid(std::string dataset1, std::string dataset2) {
    // load csr matrix
    ...
    float* cu_InMat_CSR=0;
    cudaStat4 = cudaMalloc((void**)&cu_InMat_CSR, num_cols * num_cols_b * sizeof(float));
    float* cu_OutMat_CSR=0;
    cudaStat5 = cudaMalloc((void**)&cu_OutMat_CSR, num_rows * num_cols_b * sizeof(float));
    ...
    // memcpy from host to device
    cudaStat4 = cudaMemcpy(cu_InMat_CSR, InMat, num_cols * num_cols_b * sizeof(float), cudaMemcpyHostToDevice);
    cudaStat5 = cudaMemcpy(cu_OutMat_CSR, OutMat, num_rows * num_cols_b * sizeof(float), cudaMemcpyHostToDevice);
    ...
    cusparseHandle_t handle_CSR = NULL;
    cusparseSpMatDescr_t matA_CSR;
    cusparseDnMatDescr_t matX_CSR, matY_CSR;
    void* dBuffer_CSR = NULL;
    size_t bufferSize_CSR = 0;
    
    CHECK_CUSPARSE( cusparseCreate(&handle_CSR) )
    // Create sparse matrix
    CHECK_CUSPARSE( cusparseCreateCsr(&matA_CSR, num_rows, num_cols, nnz,
                                      cu_csrRowPtr, cu_csrColInd, cu_csrVal,
                                      CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                      CUSPARSE_INDEX_BASE_ONE, CUDA_R_32F) )
    
    // Create dense vector input
    CHECK_CUSPARSE( cusparseCreateDnMat(&matX_CSR, num_cols, num_cols_b, num_cols, cu_InMat_CSR, CUDA_R_32F, CUSPARSE_ORDER_COL) )
    // Create dense vector output
    CHECK_CUSPARSE( cusparseCreateDnMat(&matY_CSR, num_rows, num_cols_b, num_rows, cu_OutMat_CSR, CUDA_R_32F, CUSPARSE_ORDER_COL) )

    // allocate an external buffer if needed
    CHECK_CUSPARSE( cusparseSpMM_bufferSize(
                                 handle_CSR, CUSPARSE_OPERATION_NON_TRANSPOSE, 
				 CUSPARSE_OPERATION_NON_TRANSPOSE, 
                                 &csr_alpha, matA_CSR, matX_CSR, &csr_beta, matY_CSR, CUDA_R_32F,
                                 CUSPARSE_SPMM_ALG_DEFAULT, &bufferSize_CSR) )

    CHECK_CUDA( cudaMalloc(&dBuffer_CSR, bufferSize_CSR) )

    // execute SpMV
    CHECK_CUSPARSE( cusparseSpMM(handle_CSR, CUSPARSE_OPERATION_NON_TRANSPOSE,
			         CUSPARSE_OPERATION_NON_TRANSPOSE,
                                 &csr_alpha, matA_CSR, matX_CSR, &csr_beta, matY_CSR, CUDA_R_32F,
                                 CUSPARSE_SPMM_ALG_DEFAULT, dBuffer_CSR) )
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    // ELL kernel
    ...
    cutensorElementwiseBinary(&handle,
                (void*)&alpha, cu_OutMat_CSR, &descIn1, modeIn1.data(),
    ...
    std::cout << "CSR nnz is " << nnz << " and num_cols is " << num_cols << std::endl;
    ...
    CHECK_CUSPARSE( cusparseDestroySpMat(matA_CSR) )
    CHECK_CUSPARSE( cusparseDestroyDnMat(matX_CSR) )
    CHECK_CUSPARSE( cusparseDestroyDnMat(matY_CSR) )
    CHECK_CUSPARSE( cusparseDestroy(handle_CSR) )
    ...
    cudaFree(cu_InMat_CSR);
    cudaFree(cu_OutMat_CSR);
    //--------------------------------------------------------------------------
...
int main(int argc, char** argv) {
    ...
    benchmark_spmv_hybrid(dataset1, dataset2);
}
