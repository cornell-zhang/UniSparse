...
int benchmark_spmv_bsr(std::string dataset1, std::string dataset2) {
    ...
    float* InMat = (float*)malloc(sizeof(float) * ldb);
    for (int i = 0; i < ldb ; i++) {
        InMat[i] = 1.0;
    }
    float* OutMat = (float*)malloc(sizeof(float) * ldc );
    for (int i = 0; i < ldc ; i++) {
        OutMat[i] = 0.0;
    }
    ...
    cudaStat4 = cudaMalloc((void**)&cu_InMat_BSR, ldb * sizeof(float));
    float* cu_OutMat_BSR=0;
    cudaStat5 = cudaMalloc((void**)&cu_OutMat_BSR, ldc * sizeof(float));

    // memcpy from host to device
    cudaStat4 = cudaMemcpy(cu_InMat_BSR, InMat, ldb * sizeof(float), cudaMemcpyHostToDevice);
    cudaStat5 = cudaMemcpy(cu_OutMat_BSR, OutMat, ldc * sizeof(float), cudaMemcpyHostToDevice);
    ...
    CHECK_CUDA( cudaMalloc((void**)&cu_InMat_COO, ldb * sizeof(float)))
    float* cu_OutMat_COO=0;
    CHECK_CUDA( cudaMalloc((void**)&cu_OutMat_COO, ldc * sizeof(float)))

    // memcpy from host to device
    CHECK_CUDA( cudaMemcpy(cu_InMat_COO, InMat, ldb * sizeof(float), cudaMemcpyHostToDevice))
    CHECK_CUDA( cudaMemcpy(cu_OutMat_COO, OutMat, ldc * sizeof(float), cudaMemcpyHostToDevice))
    //--------------------------------------------------------------------------
    ...
    CHECK_CUSPARSE( cusparseSbsrmv( handle_BSR,
                                    dir,
    ...
    CHECK_CUSPARSE( cusparseCreateDnVec(&matX_COO, coo_num_cols, cu_InMat_COO, CUDA_R_32F) )
    // Create dense vector output
    CHECK_CUSPARSE( cusparseCreateDnVec(&matY_COO, coo_num_rows, cu_OutMat_COO, CUDA_R_32F) )

    // allocate an external buffer if needed
    CHECK_CUSPARSE( cusparseSpMV_bufferSize(
                                 handle_COO, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                 &coo_alpha, matA_COO, matX_COO, &coo_beta, matY_COO, CUDA_R_32F,
                                 CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize_COO) )

    CHECK_CUDA( cudaMalloc(&dBuffer_COO, bufferSize_COO) )

    // execute SpMV
    CHECK_CUSPARSE( cusparseSpMV(handle_COO, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                 &coo_alpha, matA_COO, matX_COO, &coo_beta, matY_COO, CUDA_R_32F,
                                 CUSPARSE_SPMV_ALG_DEFAULT, dBuffer_COO) )
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    // element-wise add
    ...
    float alpha = 1.0;
    float gamma = 1.0;
    std::vector<int> modeIn1{'a'};
    std::vector<int> modeIn2{'a'};
    std::vector<int> modeOut{'a'};
    ...
    //              NULL /* stride */,
    //              CUDA_R_32F, CUTENSOR_OP_IDENTITY));
    // CHECK_CUDA( cudaMalloc((void**)&cu_OutMat, coo_num_rows * sizeof(float)) )
    ...
    // CHECK_CUSPARSE( cusparseDestroyDnVec(matX_COO) )
    // CHECK_CUSPARSE( cusparseDestroyDnVec(matY_COO) )
