...
int benchmark_spmv_csr(std::string dataset, std::string dataset1) {
    ...
    cudaStat4 = cudaMemcpy(cu_csrVal2, csrVal1, nnz * sizeof(float), cudaMemcpyHostToDevice);
    cudaStat5 = cudaMemcpy(cu_csrRowPtr2, csrRowPtr1, (num_cols + 1) * sizeof(int), cudaMemcpyHostToDevice);
    cudaStat6 = cudaMemcpy(cu_csrColInd2, csrColInd1, nnz * sizeof(int), cudaMemcpyHostToDevice);
    if ((cudaStat1 != cudaSuccess) ||
        (cudaStat2 != cudaSuccess) ||
        (cudaStat3 != cudaSuccess) ||
	(cudaStat4 != cudaSuccess) ||
    ...
    CHECK_CUSPARSE( cusparseCreateCsr(&matB, num_cols, 1, nnz1,
                                      cu_csrRowPtr2, cu_csrColInd2, cu_csrVal2,
                                      CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                      CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F) )

    CHECK_CUSPARSE( cusparseCreateCsr(&matC, num_rows, 1, 0,
                                      NULL, NULL, NULL,
                                      CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                      CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F) )
