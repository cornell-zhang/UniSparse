...
module {
  ...
  func.func @main() {
    ...
    %a2 = sparlay.convert (%a1): tensor<?x?xf32, #COO> to tensor<?x?xf32, #BCOO>
    sparlay.printStorage (%a2): tensor<?x?xf32, #BCOO>
    %t_end0 = call @rtclock() : () -> f64
    %t_0 = arith.subf %t_end0, %t_start0: f64
    vector.print %t_0 : f64

    %a3 = sparlay.convert (%a2): tensor<?x?xf32, #BCOO> to tensor<?x?xf32, #COO>
