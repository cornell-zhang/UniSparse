...
#BCSC = #sparlay.encoding<{
  crdMap = #sparlay.crd<(i, j)->(j floordiv 3, i floordiv 2, i mod 2, j mod 3)>,
...
module {
  ...
  func.func @main() {
    ...
    %a2 = sparlay.convert (%a1): tensor<?x?xf32, #COO> to tensor<?x?xf32, #BCSC>
    sparlay.printStorage (%a2): tensor<?x?xf32, #BCSC>
    %t_end0 = call @rtclock() : () -> f64
    %t_0 = arith.subf %t_end0, %t_start0: f64
    vector.print %t_0 : f64

    %a3 = sparlay.convert (%a2): tensor<?x?xf32, #BCSC> to tensor<?x?xf32, #COO>
