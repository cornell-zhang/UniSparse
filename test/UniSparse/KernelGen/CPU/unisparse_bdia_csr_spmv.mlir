// unisparse-opt ./unisparse_bdia_csr_spmv.mlir -lower-struct-convert -lower-struct -dce -unisparse-codegen -lower-format-conversion | \
...
// clang++ bdia_spmv.o -L$SPLHOME/build/lib -lmlir_unisparse_runner_utils \
//     -L$LLVM_ROOT/build/lib -lmlir_runner_utils -lmlir_c_runner_utils -o bdia_spmv
...
#BDIA = #unisparse.encoding<{
  crdMap = #unisparse.crd<(i,j)->(i floordiv 50, j minus i, i mod 50)>,
  compressMap = #unisparse.compress<fuse(0), trim(1,1)>
}>
...
module {
  ...
  func.func @main() {
    ...
    %S_1 = unisparse.decompose_BDIA %A_1, %blockSize, %thres_1 : tensor<?x?xf32, #COO>, i32, f32 to 
          !unisparse.struct< tensor<?x?xf32,#COO>, tensor<?x?xf32,#BDIA> >
    ...
    %D_0 = unisparse.convert(%B_0) : tensor<?x?xf32, #COO> to tensor<?x?xf32, #CSR>
    ...
    %ts_dim_i = arith.index_cast %dim1 : index to i32
    %ts_dim = arith.sitofp %ts_dim_i : i32 to f32
    %elm = arith.divf %f1, %ts_dim : f32
    // vector.print %elm : f32
    %b = scf.for %i = %c0 to %dim1 step %c1 iter_args(%t = %init_256_4) -> tensor<?xf32> {
    ...
    // block DIA SpMV
    %result1 = unisparse.bdia_spmv %D_0, %B_1, %b, %o1: 
    ...
    unisparse.release %S_1: !unisparse.struct< tensor<?x?xf32,#COO>, tensor<?x?xf32,#BDIA> >
