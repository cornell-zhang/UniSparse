\\ sparlay-opt ./sparlay_dcsr_sd2d.mlir -sparlay-codegen -lower-format-conversion -lower-struct -dce | \
...
module {
  ...
  func.func @dcsr_sparse_dense_add(%arg0: tensor<?x?xf32, #DCSR>, %arg1: tensor<?x?xf32>, %argx: tensor<?x?xf32>) -> tensor<?x?xf32> {
  ...
  func.func @main() {
    ...
    %A_2 = sparlay.fromFile (%fileName) : !Filename to tensor<?x?xf32, #COO>
    %A_ori = sparlay.copy (%A_2): tensor<?x?xf32, #COO> to tensor<?x?xf32, #COO>
    %a2 = sparlay.convert (%A_ori): tensor<?x?xf32, #COO> to tensor<?x?xf32, #DCSR>
    ...
    %o2 = bufferization.alloc_tensor(%c256, %c256) : tensor<?x?xf32>

    %2 = call @dcsr_sparse_dense_add(%a2, %b, %o2) : (tensor<?x?xf32, #DCSR>, tensor<?x?xf32>, tensor<?x?xf32>) -> tensor<?x?xf32>
...
//    bufferization.dealloc_tensor %o2 : tensor<?x?xf32>
