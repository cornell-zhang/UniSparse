...
module {
  ...
  func.func @kernel_csr_spmm(%arg0: tensor<?x?xf32, #CSC>, %arg1: tensor<?x?xf32>, %argx: tensor<?x?xf32>) -> tensor<?x?xf32> {
  ...
  func.func @main() {
    ...
    %a5 = sparlay.convert (%a4): tensor<?x?xf32, #COO> to tensor<?x?xf32, #CSC>
    %t_end4 = call @rtclock() : () -> f64
    %t_4 = arith.subf %t_end4, %t_start0: f64
    vector.print %t_4 : f64

    %t_start5 = call @rtclock() : () -> f64
    %a6 = sparlay.convert (%a5): tensor<?x?xf32, #CSC> to tensor<?x?xf32, #COO>
