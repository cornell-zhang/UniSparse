...
module {
  ...
  func.func @main() {
    ...
    %a3 = sparlay.convert (%a2): tensor<?x?xf32, #DIA> to tensor<?x?xf32, #DIAA>
    sparlay.printStorage (%a3): tensor<?x?xf32, #DIAA>
    %t_end0 = call @rtclock() : () -> f64
    %t_0 = arith.subf %t_end0, %t_start0: f64
    vector.print %t_0 : f64

    %t_start1 = call @rtclock() : () -> f64
    %a4 = sparlay.convert (%a3): tensor<?x?xf32, #DIAA> to tensor<?x?xf32, #DIA>
