...
module {
  ...
  func.func @main() {
    ...
    %a2 = sparlay.convert (%a1): tensor<?x?xf32, #COO> to tensor<?x?xf32, #CSR>
    sparlay.printStorage (%a2): tensor<?x?xf32, #CSR>
    
    %t_start0 = call @rtclock() : () -> f64
    %a3 = sparlay.convert (%a2): tensor<?x?xf32, #CSR> to tensor<?x?xf32, #DIA>
    sparlay.printStorage (%a3): tensor<?x?xf32, #DIA>
    %t_end0 = call @rtclock() : () -> f64
    %t_0 = arith.subf %t_end0, %t_start0: f64
    vector.print %t_0 : f64

    %a4 = sparlay.convert (%a3): tensor<?x?xf32, #DIA> to tensor<?x?xf32, #CSR>
    sparlay.printStorage (%a4): tensor<?x?xf32, #CSR>

    %a5 = sparlay.convert (%a4): tensor<?x?xf32, #CSR> to tensor<?x?xf32, #COO>
